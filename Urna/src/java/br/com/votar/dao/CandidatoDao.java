...
public class CandidatoDao extends BaseDao{
    ...
    public void cadastrarCandidato(String candidato, int idEleicao) throws SQLException {
        conn = DatabaseConnection.conectar();

        String query = "INSERT INTO UNINOVE.\"tb_candidato\" (\"NOME\", \"IDELEICAO\") "
                + " VALUES (?,?)";

        PreparedStatement st = conn.prepareStatement(query);
        st.setString(1, candidato);
        st.setInt(2, idEleicao);
        st.executeUpdate();

        DatabaseConnection.desconectar(conn);
    }
    
    public ArrayList<Candidato> BuscarEleicoes() throws SQLException {
        ...
        String query = "SELECT * FROM UNINOVE.\"tb_candidato\" ";
